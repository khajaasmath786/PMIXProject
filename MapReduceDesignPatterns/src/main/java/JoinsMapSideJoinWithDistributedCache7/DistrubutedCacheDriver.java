...
// Tool runner will call run method automatically	
	...
	if (args.length != 2)
	{
		 ...
		 // Input Output/DistributedCache
		 // This example gets file from hdfs so it is not required to mention in the arguments. 
	...
	try {
	// Used approach of Edureka here. Calling file from HDFS
	//DistributedCache.addCacheFile(new URI("/DistributedCache.dat"), job.getConfiguration()); //hdfs
	DistributedCache.addFileToClassPath(new Path("DistributedCache.dat"), job.getConfiguration());//classpath
	// Calling file from classpath. File is in Resources folder of maven
	//DistributedCache.addFileToClassPath(new Path("Input/DistributedCache.dat"), job.getConfiguration());
	}
    catch(Exception e){
	...
	job.setJobName("DistrubutedCache");
