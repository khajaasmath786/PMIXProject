...
public class HotandColdTemparature extends Configured implements Tool
{
...
// Tool runner will call run method automatically	
	...
	if (args.length != 2)
	{
		 ...
		 // Input/Partitioner/Input.txt Output/Partitioner
	    System.exit(-1);
	}
	 ...
	 */
	Job job = new Job();
	job.setJarByClass(HotandColdTemparature.class);
	job.setJobName("HotandColdTemparature");
	...
	job.setMapperClass(HotandColdTemparatureMapper.class);
	//job.setPartitionerClass(HashPartitioner.class);
	//job.setPartitionerClass(MaxTemparatureInYearPartitioner.class);
	//job.setCombinerClass(MaxTemparatureInYearReducer.class);
	job.setReducerClass(HotandColdTemparatureReducer.class);
    ...
    {
    	int exitCode= ToolRunner.run(new Configuration(),new HotandColdTemparature(),args);
