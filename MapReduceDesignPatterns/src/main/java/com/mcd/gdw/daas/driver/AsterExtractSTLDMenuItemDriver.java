...
public class AsterExtractSTLDMenuItemDriver extends Configured implements Tool {
	...
	public static void main(String[] args) throws Exception {Configuration conf1 = new Configuration();
		...
		int retval = ToolRunner.run(conf1,new AsterExtractSTLDMenuItemDriver(), args);

		System.out.println(" return value : " + retval);
		// InputAsterExtractSTLDMapper Output InputAsterExtractMenuItem
	...
	public int run(String[] argsAll) throws Exception {
		...
		try{
			...
			job.setJarByClass(AsterExtractSTLDMenuItemDriver.class);
			
			job.setMapperClass(AsterExtractSTLDMenuItemMapper.class);
			...
			String cacheMenuItemFile = args[2];
			DistributedCache.addCacheFile(new Path(cacheMenuItemFile).toUri(),job.getConfiguration());
